...
from conv_models import GCN_node_classification, GCN_graph_classification
...
parser.add_argument('--nb_nodes', type=int, default=200,
                    help='nb for graph nodes for SBMs; default is 20')
parser.add_argument('--nb_graphs', type=int, default=20,
                    help='nb for SBMs graphs; default is 20')
parser.add_argument('--p', type=float, default=None,
                    help='probability p for SBMs; default is None')
parser.add_argument('--q', type=float, default=None,
                    help='probability q for SBMs; default is None')
...
parser.add_argument('--exp_lr', type=float, default=0.005,
...
exps = 40
# nb_graphs = exps
expfolder = osp.join(args.outdir, args.dataset + '_' + str(args.nb_nodes) + 'nodes_'+str(exps)+'graphs_' + str(int(args.p*100)) + '_' + str(int(args.q*100)))
...
adj_list, features_list, labels_list, idx_train_list, idx_val_list, idx_test_list = \
    load_data(args.datadir, args.dataset, args.nb_nodes, args.nb_graphs, args.p, args.q, device)
exps = len(labels_list)

torch.save(adj_list, osp.join(expfolder,f'adj_list.pth'))
# for i in range(exps):
    # print(adj_list[i].sum()/adj_list[0].shape[0])

# exps = 15
for exp in trange(exps):
    adj, features, labels, idx_train, idx_val, idx_test = \
        adj_list[exp], features_list[exp], labels_list[exp], idx_train_list[exp], idx_val_list[exp], idx_test_list[exp]
    ...
    model = GCN_node_classification(input_dim=features.shape[1],
                ...
                dropout=args.dropout,adj=adj, genlap=args.genlap).to(device)
    ...
    for epoch in range(1,args.epochs+1):
        ...
        torch.save(states, osp.join(expfolder,f'model_states_exp={exp+1}.pth'))
